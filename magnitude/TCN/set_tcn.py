...
causal = False
input_size = 257 # frequency
hidden_size = 256
num_channels = [hidden_size, hidden_size, hidden_size, input_size]
kernel_size = 3

model = TCN(input_size, input_size, num_channels, kernel_size, causal).to(device)
criterion = nn.MSELoss()
optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9)
scheduler = ReduceLROnPlateau(optimizer=optimizer, factor=0.2, patience=2, cooldown=2)
